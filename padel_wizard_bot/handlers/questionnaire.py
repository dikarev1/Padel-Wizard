...
async def _start_hits_checklist(
    ...
    bot_instance = cast(Bot, bot_instance)
...
async def on_business_connection(
    business_connection: BusinessConnection, state: FSMContext
) -> None:
...
def register_update_observers(dispatcher: Dispatcher) -> None:
    """Attach update-level handlers that aiogram doesn't expose via router helpers."""

    dispatcher.observers["update"].register(
        on_checklist_update,
        HasChecklistDone(),
    )
